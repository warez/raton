...
global $raton_dir;
require_once($raton_dir["MODEL"] . "Filter.php");
require_once($raton_dir["SERVICE"] . "BaseRestService.php");
require_once( $raton_dir["DAO"] . "FilterDao.php");

class FilterRestService extends BaseRestService {

    function __construct($restController)
    {
        parent :: __construct($restController, new FilterDao());
    }
    ...
    function prepareForDb($filter) {
        ...
        $mandatory = parent::getProp("mandatory", $filter);
        if($mandatory != null) {
            parent::setProp("mandatory", $filter, $mandatory);
        }

        $id_type = parent::getProp("id_type", $filter);
        if($id_type != null) {
            parent::setProp("id_type", $filter, $id_type);
        }
    ...
    function searchFilterByCategory($request) {
        try {
            ...
            $ret = $this->dao->searchFilterByCategory(
                intval($catId),
                intval($pageSize),
                intval($page)
            );
            ...
            return new WP_Error( "Get Filter by category error" , __( $e->getMessage() ), array( 'status' => 500 ) );
